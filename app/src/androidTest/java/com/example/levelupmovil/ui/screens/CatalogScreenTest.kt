...
class CatalogScreenTest {
    ...
    private val catalogViewModel = mockk<CatalogViewModel>(relaxed = true)
    ...
    fun catalog_shows_products_in_grid() {
        ...
        val fakeUser = UserData("Test User", "test@mail.com", "ROL_CLIENTE", "", "", "")

        every { catalogViewModel.filteredProducts } returns MutableStateFlow(fakeProducts)
        every { catalogViewModel.selectedCategory } returns MutableStateFlow(null)
        every { catalogViewModel.isRefreshing } returns MutableStateFlow(false)
        every { profileViewModel.userData } returns MutableStateFlow(fakeUser)
